	...
	populateSummaries(dataObject) {
		...
		for (let i in dataObject.list) {
			...
			if (!document.getElementById(`five-day-forecast-${dayOfWeek}`)) {
				...
				document
					.getElementById(`five-day-forecast-sections`)
					.appendChild(newDiv);
				...
				if (dayNumber <= 5) {
					...
					document
						.getElementById(`div-day-${dayNumber}`)
						.appendChild(dayAverageSummaryForecast);

					document.getElementById(`five-day-forecast-${dayOfWeek}`).innerHTML =
						fiveDayHTML;
					document.getElementById(
						`day-${dayOfWeek}`
					).textContent = `${dayOfWeek}`;
					document
						.getElementById(`show-hide-button-${dayOfWeek}`)
			...
			most_common_weather["icon"] = mode(
				most_common_weather_obj_arrays["icon"]
			);
			most_common_weather["weather_desc"] = mode(
				most_common_weather_obj_arrays["weather_desc"]
			);
			...
			document.getElementById(
				`high-temp-${dayOfWeek}`
			).textContent = `\u00A0\u00A0${Math.floor(high_temp_max)}`;
			document.getElementById(
				`low-temp-${dayOfWeek}`
			).textContent = `\u00A0\u00A0${Math.floor(low_temp_min)}`;
			document.getElementById(
				`weatherImage-${dayOfWeek}`
			).src = `http://openweathermap.org/img/wn/${most_common_weather["icon"]}@2x.png`;
			document.getElementById(
				`weathDesc-${dayOfWeek}`
			).textContent = `${most_common_weather["weather_desc"]}`;
			document.getElementById(
				`windSpeed-${dayOfWeek}`
			).textContent = `\u00A0\u00A0${Math.ceil(average_wind_speed)}`;
			document.getElementById(
				`show-hide-button-${dayOfWeek}`
			).innerHTML = `show/hide 3-hour forecast`;
			document
				.getElementById(`show-hide-button-${dayOfWeek}`)
			...
			document.getElementById(`high-temp-${i}`).parentElement.style =
				"grid-row: 2/3; font-size: 2em; margin-left: 0;";
			document.getElementById(
				`high-temp-${i}`
			).parentElement.innerHTML = `<span id="high-temp-${i}"></span>°${this.degreeSymbol}`;
			document.getElementById(
				`high-temp-${i}`
			).textContent = `\u00A0\u00A0${Math.floor(temp_max)}`;
			document.getElementById(
				`low-temp-${i}`
			).parentElement.parentElement.style = "grid-template-rows: auto auto;";
		...
		function updateSlidePosition() {
			for (var i = 1; i < 6; i++) {
				document
					.getElementById(`div-day-${i}`)
					.classList.remove(`active-slide`);
				document
					.getElementById(`div-day-${i}`)
					.classList.remove(`hidden-slide`);
