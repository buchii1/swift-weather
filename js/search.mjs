...
searchInput.addEventListener("input", async (event) => {
	...
	const response = await fetch(
		`https://api.openweathermap.org/data/2.5/weather?q=${value}&appid=850f85405ab713d96880a077b3067953&units=metric`
	);
	...
	suggestList.innerHTML = suggestions
		.map((suggestion) => `<li class="suggestion">${suggestion}</li>`)
		.join("");
...
export async function getLocation() {
	...
	if (!location.match(/^[a-zA-Z\s]+$/)) {
		...
		const response = await fetch(
			`https://api.openweathermap.org/data/2.5/weather?q=${location}&appid=850f85405ab713d96880a077b3067953`
		);
		const data = await response.json();
