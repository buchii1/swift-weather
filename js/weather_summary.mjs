...
export default class WeatherSummary {
	...
	populateSummary(dataObject) {
		...
		document.getElementById(
			"currentTemp"
		).textContent = `\u00A0\u00A0${Math.floor(temp)}`;
		document.getElementById(
			"high-temp"
		).textContent = `\u00A0\u00A0${Math.floor(temp_max)}`;
		document.getElementById("low-temp").textContent = `\u00A0\u00A0${Math.floor(
			temp_min
		)}`;
		document.getElementById("humidity").textContent = `\u00A0\u00A0${humidity}`;
		document.getElementById("windSpeed").textContent = `\u00A0\u00A0${Math.ceil(
			speed
		)}`;
